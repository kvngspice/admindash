...
const BookingDetails = () => {
  ...
  const fetchBookingDetails = async () => {
    try {
      const response = await fetch(`${config.API_URL}/api/admin/bookings/${bookingId}/detail/`);
  ...
  const handleStatusUpdate = async (newStatus) => {
    try {
      const response = await fetch(`${config.API_URL}/api/admin/bookings/${bookingId}/update-status/`, {
        method: 'POST',
        headers: {
